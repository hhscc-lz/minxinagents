...
class TestUserMessageModeRendering:
    ...
    def test_bang_prefix_renders_as_normal_message(self) -> None:
        """`UserMessage('!ls')` should render as a normal user message."""
...
class TestQueuedUserMessageModeRendering:
    ...
    def test_bang_prefix_renders_as_normal_queued_message(self) -> None:
        """`QueuedUserMessage('!ls')` should render as a normal queued message."""
...
class TestMessageClickBehavior:
    """Tests for click behavior on message widgets."""

    def test_tool_call_message_click_without_output_is_noop(self) -> None:
        """ToolCallMessage click without output should not toggle or notify."""
    ...
    def test_tool_call_message_click_with_output_toggles(self) -> None:
        ...
        with patch.object(msg, "toggle_output") as mock_toggle:
            msg.on_click(event)

        event.stop.assert_called_once()
        mock_toggle.assert_called_once()

    def test_app_message_click_opens_link_without_notify(self) -> None:
        """AppMessage click should only handle links."""
        msg = AppMessage("test")
        event = MagicMock()
        event.style = Style()

        with patch(_WEBBROWSER_OPEN):
            msg.on_click(event)
