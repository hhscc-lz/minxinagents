...
InputMode = Literal["normal", "command"]
...
写入位置：用户级 `~/.deepagents/{assistant_id}/AGENTS.md`
...
创建位置：`~/.deepagents/{assistant_id}/skills/<名称>/SKILL.md`
...
1. **先读取**现有的 `~/.deepagents/{assistant_id}/AGENTS.md` 和 `~/.deepagents/{assistant_id}/skills/` 目录
...
class DeepAgentsApp(App):
    ...
    async def on_chat_input_submitted(self, event: ChatInput.Submitted) -> None:
        ...
        # If the agent is running or server is still starting up, enqueue
    ...
    async def _handle_command(self, command: str) -> None:
        ...
        if cmd == "/反馈":
            ...
            # 注入实际路径，避免 Agent 猜错用户目录
            from pathlib import Path
            user_base = Path.home() / ".deepagents" / self._assistant_id
            agents_md_path = user_base / "AGENTS.md"
            skills_path = user_base / "skills"
            final_prompt = REMEMBER_PROMPT.replace(
                "~/.deepagents/{assistant_id}/AGENTS.md",
                str(agents_md_path),
            ).replace(
                "~/.deepagents/{assistant_id}/skills/",
                str(skills_path) + "/",
            ).replace(
                "~/.deepagents/{assistant_id}/skills/<名称>/SKILL.md",
    ...
    def action_quit_or_interrupt(self) -> None:
        ...
        Priority order:
        1. If approval menu is active, reject it
    ...
    def action_interrupt(self) -> None:
        ...
        3. If input is in command mode, exit to normal mode
        4. If approval menu is active, reject it
        ...
        # Close completion popup or exit command mode
        if self._chat_input:
    ...
    def exit(
        ...
        # active workers before the event loop is torn down.
        self._discard_queue()
        if self._agent_running and self._agent_worker:
